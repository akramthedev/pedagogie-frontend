  ...
  },
  {
    ...
    cell: (row) => (
      <input
        ...
        value={row.note || ""}
        onChange={(e) => handleNoteChange(row.Etd_Id, e.target.value)}
        placeholder="Note"
        min="0"
        max="20"
        style={{ width: "80px" }}
...
const SaisieNotesGeneral = ({ Cls_Id, Ens_Id, Mat_Id, Exam_Id }) => {
  ...
  const handleNoteChange = (Etd_Id, note) => {
    if (note >= 0 && note <= 20) {
      setData((prevData) =>
        prevData.map((student) =>
          student.Etd_Id === Etd_Id ? { ...student, note } : student
        )
      );
    }
  };
  ...
  const handleSubmit = async () => {
    const formattedData = data.map((student) => ({
      Etd_Id: student.Etd_Id || null,
      Cls_Id: Cls_Id || null,
      Ens_Id: Ens_Id || null,
      Mat_Id: Mat_Id || null,
      Exam_Id: Exam_Id || null, // ID de l'examen (ou évaluation)
    ...
    try {
      await axios.post(
        "http://localhost:3002/api/enseignants/notes",
        formattedData
      );
  ...
  return (
    <Fragment>
      {/* <H4 attrH4={{ className: "text-muted my-3" }}>Saisie des Notes</H4> */}
      <DataTable
        data={data}
