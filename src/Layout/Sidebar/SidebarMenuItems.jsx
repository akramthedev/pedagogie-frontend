...
const SidebarMenuItems = ({ setMainMenu, sidebartoogle, setNavActive, activeClass }) => {
  ...
  // État local pour le rôle utilisateur
  const [userRole, setUserRole] = useState(localStorage.getItem("eduos_role") || "general");

  // Actualiser le rôle utilisateur depuis le localStorage lorsqu'il change
  useEffect(() => {
    const handleRoleChange = () => {
      const updatedRole = localStorage.getItem("eduos_role") || "general";
  ...
  return (
    <>
      {MENUITEMS.filter((Item) => {
        // Filtrer les groupes en fonction du rôle
        // Si le rôle est "general", afficher tous les éléments
        return (
          userRole === "general" ||
          Item.Items.some((menuItem) => menuItem.role === userRole || menuItem.role === "general")
        );
      }).map((Item, i) => (
        <Fragment key={i}>
                      ...
                      <li key={index}>
                        {childrenItem.type === "sub" ? (
                          <a
                            href="javascript"
                            className={`${
                              CurrentPath.includes(childrenItem?.title?.toLowerCase()) ? "active" : ""
                            }`}
                            onClick={(event) => {
                              event.preventDefault();
                              ...
                              <li key={key}>
                                {childrenSubItem.type === "link" ? (
                                  <Link
                                    ...
                                    className={`${
                                      CurrentPath.includes(childrenSubItem?.title?.toLowerCase()) ? "active" : ""
                                    }`}
